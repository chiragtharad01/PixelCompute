let playingContainer = document.querySelector(".grid-container");
Array.from({length:30},(i)=>i-1).forEach((item)=>{
    let newSquare = document.createElement("div");
    newSquare.classList.add("grid-item");
    playingContainer.appendChild(newSquare)
})
...
const words = [
    ...
    let count = 0;
    let guessGrid = document.querySelectorAll('.grid-item');
    const userInputEle= document.getElementById("user-input")
    window.addEventListener('keydown',(event)=>{
        if(event.key == 'Enter') checkGuess()
    })
    const checkGuess = () =>{
        let userInput = userInputEle.value.trim().toLowerCase();
        const feedback = document.querySelector(".feedback");
        console.log(userInput)
        if(!userInput || userInput.length!=5){
            feedback.textContent="Enter valid input";
            return;
        }
    ...
    const markCells = (userInput) =>{
        let localWordToGuess = [...wordToGuess];
        let userInputArray = userInput.split('');
        let arr = [0,1,2,3,4]
        arr.forEach((index)=>{
            if(userInputArray[index] != "" && userInputArray[index] == (localWordToGuess[index])){
                console.log(index);
                guessGrid[index+(5*count)].style.backgroundColor = "green";
                localWordToGuess[index] = "";
                userInputArray[index] = "";
            } 
            guessGrid[index+(5*count)].textContent = userInput[index].toUpperCase();
        })
        arr.forEach((index)=>{
            console.log("Entered")
            if(userInputArray[index]!="" && localWordToGuess.includes(userInputArray[index])){
                console.log("Enteredsffdsd")
                guessGrid[index+(5*count)].style.backgroundColor = "yellow";
                guessGrid[index+(5*count)].style.color = "black";
                let ind = localWordToGuess.indexOf(userInputArray[index])
                userInputArray[index] = "";
                console.log(ind)
                localWordToGuess[ind] = "";
            }
        })
        arr.forEach((index)=>{
            if(userInputArray[index]!=""){
                guessGrid[index+(5*count)].style.backgroundColor = "gray";
            }
        })
    }